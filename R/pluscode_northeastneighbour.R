#' @title Pluscode Northeast Neighbour
#'
#' @description This package retrieves the north-easterly neighbour of a valid pluscode, with a precision of 2, 4, 8, or 10 excluding the plus sign (which can be included)
#'
#' @param pluscode A valid pluscod 2, 4, 8, or 10 characters in length excluding the plus sign (which can be included)
#'
#' @return NULL
#'
#' @examples pluscode_northeastneighbour("9C4MGC2M+H4")
#'
#' @export

pluscode_northeastneighbour <- function(pluscode) {
  ...
  n8<-if(is.na(d8)==TRUE) {
    NA
  } else if(any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & grep(d7,code)==20){
    code[1]
  } else if (any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T)) {
    code[grep(d8,code)+1]
  } else {
    code[grep(d8,code)]
  }
  ...
  n6<-if(is.na(d6)==TRUE) {
    NA
  } else if(any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & any(is.na(d8)==TRUE | grep(d8,code)==20,na.rm = T) & grep(d6,code)==20){
    code[1]
  } else if (any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & any(is.na(d8)==TRUE | grep(d8,code)==20,na.rm = T)) {
    code[grep(d6,code)+1]
  } else {
    code[grep(d6,code)]
  }
  ...
  n4<-if(is.na(d4)==TRUE) {
    NA
  } else if(any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & any(is.na(d8)==TRUE | grep(d8,code)==20,na.rm = T) & any(is.na(d6)==TRUE | grep(d6,code)==20,na.rm = T) & grep(d4,code)==20){
    code[1]
  } else if (any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & any(is.na(d8)==TRUE | grep(d7,code)==20,na.rm = T) & any(is.na(d6)==TRUE | grep(d6,code)==20,na.rm = T)) {
    code[grep(d4,code)+1]
  } else {
    code[grep(d4,code)]
  }
  ...
  n2<-if(is.na(d2)==TRUE) {
    NA
  } else if(any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & any(is.na(d8)==TRUE | grep(d8,code)==20,na.rm = T) & any(is.na(d6)==TRUE | grep(d6,code)==20,na.rm = T) & any(is.na(d4)==TRUE | grep(d4,code)==20,na.rm = T) & grep(d2,code)==20){
    code[1]
  } else if(any(is.na(d10)==TRUE | grep(d10,code)==20,na.rm = T) & any(is.na(d8)==TRUE | grep(d8,code)==20,na.rm = T) & any(is.na(d6)==TRUE | grep(d6,code)==20,na.rm = T) & any(is.na(d4)==TRUE | grep(d4,code)==20,na.rm = T)) {
    as.numeric(d2)+1
  } else {
    d2
  }
